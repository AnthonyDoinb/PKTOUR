...
public class Hook extends Thread{
	...
	JLabel jlHook=new JLabel(new ImageIcon("img/钩子.PNG"));
	//运动参量
	int v;
	double zeta=0;
	double w=0;
					...
					if(hookPos[4]!=-1) {
						...
						if(Math.sqrt((hookPos[2]-hookPos[0])*(hookPos[2]-hookPos[0])+(hookPos[3]-hookPos[1])*(hookPos[3]-hookPos[1]))<=radius) {
							...
							if(name==1) {
								...
								System.out.print(jp.iMoney1);
								int id=Mysql.exec("UPDATE gameresult SET goldres1 = "+jp.iMoney1+" WHERE id = "+jp.idd);
								...
								int id=Mysql.exec("UPDATE gameresult SET goldres2 = "+jp.iMoney2+" WHERE id = "+jp.idd);
							}
