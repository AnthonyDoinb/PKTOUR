...
public class Scene extends JPanel implements ActionListener,KeyListener{
	...
	JButton jlExit=new JButton("退出本游戏");
	...
	public Scene(int id) throws Exception{
		this.idd=id;
		myframe=new JFrame();
		myframe.setTitle("黄金矿工");
		myframe.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		myframe.setContentPane(this);
		this.setLayout(null);
		this.setOpaque(false);
		
		ImageIcon bg=new ImageIcon("img/主界面.PNG");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getActionCommand().equals("退出本游戏")) {
			gameState=1;
			try{client.sendexit();}catch(Exception w) {};
		}
		else if(e.getActionCommand().equals("进入你画我猜")) {
		
			
			
			//try{client.sendchoice(true);}catch(Exception w) {};
			myframe.setVisible(false);
			DrawingFrame bd = new DrawingFrame(this.idd);
			bd.showUI();
			//System.exit(0);
		}
	...
	public void keyReleased(KeyEvent e) {
		...
		if(e.getKeyCode()==KeyEvent.VK_W && Hhook1.state_hook==0) {
			//music.playonce();
			Hhook1.state_hook=1;
		}
	}
